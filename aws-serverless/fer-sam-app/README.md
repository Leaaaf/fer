...
```bash
fer-sam-app$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
fer-sam-app$ sam local start-api
fer-sam-app$ curl http://localhost:3000/classify_digit
```
...
```bash
fer-sam-app$ sam logs -n InferenceFunction --stack-name fer-sam-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name fer-sam-app
```
